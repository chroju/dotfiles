...
### ステップ2: 変更の分析とコミット単位の決定

3. 現在のコンテキストで編集したファイルを特定
4. **論理的に異なる変更がある場合は、複数のコミットに分割することを決定**
   - この段階ではまだ `git add` を実行しない
   - どのファイルをどのコミットに含めるかを先に決定する

### ステップ3: 各コミットの実行

5. 決定したコミット単位ごとに、以下を順に実行:
   - **コミットメッセージを生成**
   - **該当ファイルのみを `git add <files>` でステージング**
   - **`git commit -m "<生成されたメッセージ>"` でコミット**

   重要: 各コミットごとに「メッセージ生成 → add → commit」を順に実行する
   すべてのファイルを先にステージングしない（git reset が必要になるため）

6. すべてのコミットが完了したら `git push origin <current-branch>` でプッシュ
