---
description: PRを監視し、CIの失敗調査・修正、レビューコメントへの対応、マージ提案を実行
argument-hint: "[省略可: PR番号]"
allowed-tools: Bash(gh pr view:*), Bash(gh pr checks:*), Bash(gh api:*), Bash(gh pr merge:*), Bash(gh run view:*), Bash(git status:*), Bash(git diff:*), Bash(git log:*), Bash(git branch:*), Bash(git switch:*), Bash(git add:*), Bash(git commit:*), Bash(git push:*), Bash(git pull:*), Read, Glob, Grep, Edit, MultiEdit, Write, Skill(commit)
---

# タスク

PRの状態を監視し、CI失敗の修正・レビューコメントへの対応・マージ提案を実行する。
`/loop` との組み合わせ（例: `/loop 5m /watch-pr`）で定期的な監視が可能。
...
- `state` が `MERGED` → **Step 5（クリーンアップ）** へ
...
**条件**: CIが全て成功（または未設定）で、未対応のレビューコメントがある

1. `gh api repos/{owner}/{repo}/pulls/{number}/comments` でレビューコメント取得
2. `gh pr view <number> --json reviews` でレビュー情報を取得
3. 未対応コメントの判定:
   - スレッド内で最新の返信がPR作成者以外 → 未対応
...
1. 「PR #N はすべてのチェックに合格し、承認済みです。マージしますか？」と報告
...
- 現在の状態を簡潔に報告して終了:
  - チェック実行中の場合: 「CIチェックが実行中です。次の確認まで待機します。」
  - レビュー待ちの場合: 「レビュー待ちです。次の確認まで待機します。」
...
- マージ済みPRは Step 2 → Step 5 で早期終了する
